  ...
  try {
    ...
    const communityData = {
      ...
      maxMembers: document.getElementById("maxMembers").value
        ? parseInt(document.getElementById("maxMembers").value)
        : null,
      tags: document
        .getElementById("communityTags")
        .value.split(",")
        .map((tag) => tag.trim())
        .filter((tag) => tag),
    ...
    const duplicate = existingCommunities.find(
      (c) => c.name.toLowerCase() === communityData.name.toLowerCase()
    );
    if (duplicate) {
    ...
    updateUserActivity("created_community", {
