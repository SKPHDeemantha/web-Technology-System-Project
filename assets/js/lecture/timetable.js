...
class TimetableManager {
  ...
  initializeEventListeners() {
    // Modal events
    document
      .getElementById("addTimetableModal")
    ...
    // Form submission
    document
      .getElementById("saveTimetableEntry")
      ...
      if (e.ctrlKey || e.metaKey) {
        switch (e.key) {
          case "n":
            e.preventDefault();
            new bootstrap.Modal(
              document.getElementById("addTimetableModal")
            ).show();
            break;
          case "e":
            e.preventDefault();
  ...
  resetForm() {
    ...
    document.getElementById("modalTitle").innerHTML =
      '<i class="fas fa-calendar-plus me-2"></i>Add New Class';
  }
  ...
  renderClassCard(entry, index) {
    ...
    const startIndex = timeSlots.findIndex(
      (time) => this.parseTime(time) >= startTime
    );
    const endIndex = timeSlots.findIndex(
      (time) => this.parseTime(time) >= endTime
    );
  ...
  addCurrentTimeIndicator() {
    ...
    if (daySlots.length > 0) {
      ...
      const slotIndex = Math.floor(
        (currentTime * 60 * 1000 - startTime) / (slotDuration * 60 * 1000)
      );
      ...
      if (slotIndex >= 0 && slotIndex < daySlots.length) {
        ...
        const minutesIntoSlot =
          (currentTime -
            timeSlots[slotIndex].split(":").map(Number)[0] * 60 -
            timeSlots[slotIndex].split(":").map(Number)[1]) *
          60 *
          1000;
  ...
  updateStats() {
    ...
    if (nextClass) {
      document.getElementById("nextClassTime").textContent =
        nextClass.startTime;
      document.getElementById("nextClassSubject").textContent =
        nextClass.subject;
    } else {
      document.getElementById("nextClassTime").textContent = "--:--";
      document.getElementById("nextClassSubject").textContent =
  ...
  getNextClass() {
    ...
    const todayClasses = this.timetableData
      ...
      .filter((entry) => {
        const startMinutes =
          entry.startTime.split(":").map(Number)[0] * 60 +
          entry.startTime.split(":").map(Number)[1];
        return startMinutes > currentTime;
      })
      .sort((a, b) => {
        const aMinutes =
          a.startTime.split(":").map(Number)[0] * 60 +
          a.startTime.split(":").map(Number)[1];
        const bMinutes =
          b.startTime.split(":").map(Number)[0] * 60 +
          b.startTime.split(":").map(Number)[1];
  ...
  clearTimetable() {
    if (
      confirm(
        "Are you sure you want to clear all classes? This action cannot be undone."
      )
    ) {
      this.timetableData = [];
      this.saveToStorage();
  ...
  saveSettings() {
    ...
    this.settings.timeSlotDuration = parseInt(
      document.getElementById("timeSlotDuration").value
    );
