...
function initializeFileManager() {
    ...
    function renderFilesTable() {
        ...
        if (currentFilter !== 'all') {
            if (currentFilter === 'pdf') {
                ...
                filteredFiles = files.filter(file => ['jpg', 'jpeg', 'png', 'gif'].includes(file.type));
            } else if (currentFilter === 'archive') {
                filteredFiles = files.filter(file => ['zip', 'rar'].includes(file.type));
            } else if (currentFilter === 'other') {
                filteredFiles = files.filter(file => !['pdf', 'jpg', 'jpeg', 'png', 'gif', 'zip', 'rar'].includes(file.type));
            }
        }
        ...
        recentFilesTable.innerHTML = filteredFiles.map(file => {
    ...
    if (uploadFileForm) {
        uploadFileForm.addEventListener('submit', function(e) {
            ...
            const fileTags = document.getElementById('fileTags').value.split(',').map(tag => tag.trim()).filter(tag => tag);
    ...
    window.filterFiles = function(category) {
        ...
        const categoryLinks = document.querySelectorAll('#sidebar .nav-link[onclick*="filterFiles"]');
