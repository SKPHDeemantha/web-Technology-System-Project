...
function updateDarkModeIcons(theme) {
  const darkModeIcons = document.querySelectorAll(
    "#darkModeToggle i, #darkModeToggleSidebar i"
  );
    ...
    if (theme === "dark") {
      ...
      if (buttonText) {
        buttonText.innerHTML =
          '<i class="fas fa-sun me-2"></i> Switch to Light Mode';
      }
    } else {
      icon.className = "fas fa-moon fa-lg";
      if (buttonText) {
        buttonText.innerHTML =
          '<i class="fas fa-moon me-2"></i> Switch to Dark Mode';
      }
    }
  });
}
...
let assignments = [
  {
    ...
    description:
      "Complete the programming exercises covering variables, data types, and basic operations.",
  ...
  },
  {
    ...
    description:
      "Implement a linked list data structure with all basic operations and write comprehensive tests.",
  ...
  },
  {
    ...
    description:
      "Write a research paper discussing ethical considerations in machine learning applications.",
  ...
  },
  {
    ...
    description:
      "Multiple choice quiz covering software development methodologies and project management.",
...
function renderAssignmentsList() {
  ...
  assignmentsList.innerHTML = assignments
    .map(
      (assignment) => `
        <div class="assignment-item card mb-3 ${assignment.status}" data-id="${
        assignment.id
      }">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-start mb-2">
                    <div class="flex-grow-1">
                        <div class="d-flex align-items-center mb-2">
                            <span class="badge bg-${getStatusBadgeClass(
                              assignment.status
                            )} me-2">${assignment.status.toUpperCase()}</span>
                            <span class="badge bg-secondary me-2">${assignment.type.toUpperCase()}</span>
                            ${
                              assignment.allowLate
                                ? '<span class="badge bg-warning me-2"><i class="fas fa-clock me-1"></i>LATE SUBMISSION</span>'
                                : ""
                            }
                        </div>
                    ...
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end">
                            <li><a class="dropdown-item" href="#" onclick="viewSubmissions(${
                              assignment.id
                            })"><i class="fas fa-eye me-2"></i>View Submissions</a></li>
                            <li><a class="dropdown-item" href="#" onclick="gradeAssignment(${
                              assignment.id
                            })"><i class="fas fa-clipboard-check me-2"></i>Grade</a></li>
                            <li><a class="dropdown-item" href="#" onclick="editAssignment(${
                              assignment.id
                            })"><i class="fas fa-edit me-2"></i>Edit</a></li>
                            <li><a class="dropdown-item" href="#" onclick="duplicateAssignment(${
                              assignment.id
                            })"><i class="fas fa-copy me-2"></i>Duplicate</a></li>
                            <li><hr class="dropdown-divider"></li>
                            <li><a class="dropdown-item text-danger" href="#" onclick="deleteAssignment(${
                              assignment.id
                        ...
                        <div class="assignment-details text-end">
                            <small class="text-muted">
                                <i class="fas fa-star me-1"></i>${
                                  assignment.points
                                } points (${assignment.weight}%)
                                <br>
                                <i class="fas fa-clock me-1"></i>Due: ${formatDateTime(
                                  assignment.dueDate
                                )}
                                ${
                                  assignment.averageScore > 0
                                    ? `<br><i class="fas fa-chart-line me-1"></i>Avg: ${assignment.averageScore}%`
                                    : ""
                                }
                            </small>
                        </div>
                ...
                <div class="grading-progress">
                    <div class="d-flex justify-content-between align-items-center mb-1">
                        ...
                        <small class="text-muted">${assignment.graded}/${
                        assignment.submissions
                      }</small>
                    </div>
  ...
  if (gradeButton) {
    gradeButton.addEventListener("click", function () {
      const gradingAssignment = assignments.find(
        (a) => a.status === "grading" || a.graded < a.submissions
      );
...
function initializeDeadlines() {
  ...
  deadlinesList.innerHTML = upcomingAssignments
    .map((assignment) => {
...
function createAssignment() {
  ...
  const submissionType = document.getElementById(
    "assignmentSubmissionType"
  ).value;
  const allowLate = document.getElementById("allowLateSubmission").checked;
  const latePenalty = allowLate
    ? parseInt(document.getElementById("lateSubmissionPenalty").value)
    : 0;
  const lateDays = allowLate
    ? parseInt(document.getElementById("lateSubmissionDays").value)
    : 0;
  ...
  const newAssignment = {
    ...
    course: document.querySelector(
      `#assignmentCourse option[value="${course}"]`
    ).text,
    type,
    points,
    weight,
...
function gradeAssignment(id) {
  ...
  gradingContent.innerHTML = `
        <div class="grading-interface">
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="mb-3">
                        <label for="gradeScore" class="form-label">Score (out of ${
                          assignment.points
                        })</label>
                        <input type="number" class="form-control" id="gradeScore" min="0" max="${
                          assignment.points
                        }" value="${Math.floor(assignment.points * 0.8)}">
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="mb-3">
  ...
  // Show grading modal
  const gradeModal = new bootstrap.Modal(
    document.getElementById("gradeAssignmentModal")
  );
  gradeModal.show();
}
...
function filterAssignments(filter) {
  ...
  assignmentItems.forEach((item) => {
    ...
    switch (filter) {
      ...
      case "completed":
        item.style.display =
          assignment.status === "completed" ? "block" : "none";
        break;
      default:
        item.style.display = "block";
    }
...
function searchAssignments(query) {
  ...
  assignmentItems.forEach((item) => {
    ...
    const assignmentDescription = item
      .querySelector(".card-text")
      .textContent.toLowerCase();
    const assignmentCourse = item
      .querySelector(".assignment-meta")
      .textContent.toLowerCase();

    const matches =
      assignmentTitle.includes(searchTerm) ||
