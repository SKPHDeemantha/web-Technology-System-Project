...
function initializeCommunitiesPage() {
  ...
  let communities = dataManager.getItem("communities", [
    {
      ...
      description:
        "A community for computer science students to collaborate on projects and share knowledge.",
      category: "academic",
      type: "public",
      members: 45,
      events: 3,
    ...
    },
    {
      ...
      description:
        "Exploring the world of data science, machine learning, and AI together.",
      category: "technical",
      type: "public",
      members: 32,
      events: 2,
    ...
    },
    {
      ...
      description:
        "A creative space for artists, designers, and anyone interested in visual arts.",
      category: "cultural",
      type: "public",
      members: 28,
      events: 4,
  ...
  // DOM Elements
  const recentCommunitiesTable = document.getElementById(
    "recentCommunitiesTable"
  );
      ...
      row.innerHTML = `
                ...
                <td>
                    <div class="btn-group btn-group-sm">
                        <button class="btn btn-outline-primary" onclick="viewCommunity(${
                          community.id
                        })">
                            <i class="fas fa-eye"></i>
                        </button>
                        <button class="btn btn-outline-secondary" onclick="joinCommunity(${
                          community.id
                        })">
  ...
  function updateDashboardStats() {
    ...
    const totalEvents = communities.reduce(
      (sum, community) => sum + community.events,
      0
    );
    const totalDiscussions = communities.reduce(
      (sum, community) => sum + community.discussions,
      0
    );
    ...
    if (activeMembersEl)
      activeMembersEl.textContent = totalMembers.toLocaleString();
  ...
  window.joinCommunity = function (communityId) {
    ...
    if (community) {
      ...
      const isMember = community.memberList.some(
    ...
    searchCommunity.addEventListener("input", function () {
      ...
      if (query.length > 2) {
        ...
        if (filteredCommunities.length > 0) {
          results.innerHTML = filteredCommunities
            .map(
              (community) => `
                        <div class="search-result-item p-3 border-bottom">
                            <div class="d-flex justify-content-between align-items-start">
                                <div class="flex-grow-1">
                                    ...
                                    <p class="mb-2 text-muted small">${community.description.substring(
                                      0,
                                      100
                                    )}...</p>
                                    <div class="d-flex align-items-center">
                                ...
                                <button class="btn btn-sm btn-purple" onclick="joinCommunity(${
                                  community.id
                                })">
