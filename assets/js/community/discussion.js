...
function initializeDiscussionsPage() {
  ...
  let discussions = JSON.parse(
    localStorage.getItem("communityDiscussions")
  ) || [
    {
      id: 1,
    ...
    },
    {
      ...
      content:
        "Reminder about the upcoming project submission deadlines for this semester.",
      category: "academic",
  ...
  function renderDiscussions() {
    ...
    discussionsContainer.innerHTML = discussionsToShow
      .map((discussion) => {
        ...
        return `
                <div class="card discussion-card">
                    <div class="card-body">
                        ...
                        <div class="mt-3">
                            <button class="btn btn-sm btn-outline-purple me-1" onclick="viewDiscussion(${
                              discussion.id
                            })">
                                <i class="fas fa-eye me-1"></i> View
                            </button>
                            <button class="btn btn-sm btn-outline-success me-1" onclick="likeDiscussion(${
                              discussion.id
                            })">
                                <i class="fas fa-heart me-1"></i> Like
                            </button>
                            <button class="btn btn-sm btn-outline-info me-1" onclick="openReplyModal(${
                              discussion.id
                            })">
  ...
  window.viewDiscussion = function (discussionId) {
    ...
    if (discussion) {
      ...
      // Populate view modal
      document.getElementById("viewDiscussionTitle").textContent =
        discussion.title;
      ...
      document.getElementById("viewDiscussionContent").innerHTML =
        discussionContent;
      ...
      const modal = new bootstrap.Modal(
        document.getElementById("viewDiscussionModal")
      );
      modal.show();
  ...
  window.editDiscussion = function (discussionId) {
    ...
    if (discussion && discussion.author === "Student User") {
      ...
      document.getElementById("editDiscussionContent").value =
        discussion.content;
      document.getElementById("editDiscussionCategory").value =
        discussion.category;
      document.getElementById("editDiscussionTags").value =
      ...
      // Show edit modal
      const modal = new bootstrap.Modal(
        document.getElementById("editDiscussionModal")
      );
      modal.show();
    }
  };
  ...
  window.deleteDiscussion = function (discussionId) {
    ...
    if (discussion && discussion.author === "Student User") {
      if (
        confirm(
          `Are you sure you want to delete the discussion "${discussion.title}"?`
        )
      ) {
  ...
  function updateStatistics() {
    ...
    const mostLiked =
      discussions.length > 0 ? Math.max(...discussions.map((d) => d.likes)) : 0;

    document.getElementById("totalDiscussionsCount").textContent =
      totalDiscussions;
  ...
  if (editDiscussionForm) {
    editDiscussionForm.addEventListener("submit", function (e) {
      ...
      if (discussion && discussion.author === "Student User") {
        ...
        discussion.category = document.getElementById(
          "editDiscussionCategory"
        ).value;
        discussion.tags = document
          .getElementById("editDiscussionTags")
          .value.split(",")
          .map((tag) => tag.trim())
          .filter((tag) => tag);
        ...
        const newReply = {
          id:
            discussion.replies.length > 0
              ? Math.max(...discussion.replies.map((r) => r.id)) + 1
              : 1,
